...
public class InventoryStockServiceImpl implements InventoryStockService {
    ...
    public InventoryStockResponseDTO getById(
        ...
        InventoryStock stock = stockRepository.findById(inventoryId)
                .orElseThrow(() -> {
                    ...
                    return new RuntimeException("Inventory not found: " + inventoryId);
                });
    ...
    public AvailabilityResponseDTO availability(
        ...
        InventoryStock stock = stockRepository
            ...
            .orElseThrow(() -> {
                log.warn("Inventory not found: variantId={}, location={}",
                    productVariantId,
                    locationCode
                );
    ...
    public InventoryStockResponseDTO update(
        ...
        InventoryStock stock = stockRepository.findById(inventoryId)
            .orElseThrow(() -> {
                log.warn("Inventory not found for update: inventoryId={}",
                    inventoryId
                );
                return new RuntimeException("Inventory not found");
            });
    ...
    public InventoryStockResponseDTO adjust(
        ...
        InventoryStock stock = stockRepository.findByIdForUpdate(inventoryId)
            .orElseThrow(() -> {
                log.warn("Inventory not found for adjustment: inventoryId={}",
                    inventoryId
                );
                return new RuntimeException("Inventory not found");
            });
        ...
        if (type.equals("IN")) {
            stock.setQuantityOnHand(
                stock.getQuantityOnHand() + request.getQuantity()
            );
            movementService.createMovement(
                stock.getProductVariantId(),
                stock.getLocationCode(),
            ...
            log.debug("Stock increased: inventoryId={}, quantity={}",
                inventoryId,
                request.getQuantity()
            );
        }
        // STOCK OUT
            ...
            int newQuantity = stock.getQuantityOnHand() - request.getQuantity();
            /*
             * Never allow physical stock to go below already reserved quantity.
             */
            if (newQuantity < stock.getQuantityReserved()) {
                log.warn("Stock adjustment rejected: inventoryId={}, " + "newQuantity={}, reserved={}",
                    inventoryId,
                    newQuantity,
                    stock.getQuantityReserved()
                );
        ...
        InventoryStock updated = stockRepository.save(stock);
        log.info(
            "Inventory adjustment completed: inventoryId={}, " + "quantityOnHand={}, status={}",
            inventoryId,
